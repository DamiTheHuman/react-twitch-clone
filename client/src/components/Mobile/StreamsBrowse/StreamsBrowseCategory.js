import React from "react";
import Pill from "../../common/Pill";
import LiveStreamCard from "../../cards/LiveStreamCard";
class StreamsBrowseCategory extends React.Component {
  render() {
    var Streams = [0, 1, 2, 3, 4, 5, 6, 7, 8];
    const renderLiveStreams = Streams.map((stream, index) => {
      return (
        <React.Fragment key={index}>
          <LiveStreamCard />
        </React.Fragment>
      );
    });
    return (
      <div className="streams-browse-category px-2">
        <div className="title-card py-4">
          <div className="flex flex-row items-center space-x-2">
            {/* Category thumbnail*/}
            <img
              src="http://via.placeholder.com/80x106"
              className="h-auto"
              alt="A Category"
            />
            {/*Category Info*/}
            <div>
              <h3 className="text-lg font-semibold"> Just Chatting </h3>
              <p className="text-sm">
                <span className="font-semibold">315K</span> Viewers ·{" "}
                <span className="font-semibold">12.5M</span> Followers
              </p>
              <Pill content="Action" />
            </div>
          </div>
        </div>
        <div className="active-streams grid lg:grid-cols-3  sm:grid-cols-2 grid-cols-1  gap-2">
          {renderLiveStreams}
        </div>
      </div>
    );
  }
}
export default StreamsBrowseCategory;
